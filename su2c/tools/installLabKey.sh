...
BRANCH=DISCVR161_Installers
PROJECT_NAME=LabKey16.1DISCVR
...
#stop tomcat first to avoid issues by deleting modules out from under it
service tomcat stop
...
mv ./teamcity.labkey.org/guestAuth/repository/download/LabKey_${BRANCH}/.lastSuccessful/${MODULE_DIST_NAME}/*.zip ./
...
if [ $# -eq 0 ]; then
    ...
    mv ./teamcity.labkey.org/guestAuth/repository/download/LabKey_${BRANCH}/.lastSuccessful/${DIST_NAME}/*.gz ./
    rm -Rf ./teamcity.labkey.org
