...
CREATE PROCEDURE openldapsync.handleUpgrade AS
BEGIN
IF EXISTS( SELECT name from sys.tables WHERE name = 'ldapSyncMap' AND schema_id = (SELECT schema_id FROM sys.schemas WHERE name = 'ldk' ) )
  BEGIN
...
EXEC openldapsync.handleUpgrade
GO

DROP PROCEDURE openldapsync.handleUpgrade