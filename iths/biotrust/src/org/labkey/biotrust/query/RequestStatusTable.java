 ...
 * Date: 1/14/13
 */
public class RequestStatusTable extends FilteredTable<BioTrustQuerySchema>
{
    public RequestStatusTable(TableInfo ti, BioTrustQuerySchema schema)
    {
        super(ti, schema);
        ...
        List<FieldKey> defaultColumns = new ArrayList<>(Arrays.asList(
                ...
                FieldKey.fromParts("LockedState"),
                FieldKey.fromParts("ApprovalState"),
                FieldKey.fromParts("FinalState")
        ));
    ...
    public static List<String> getLockedStates(Container c, User user)
    {
        UserSchema schema = QueryService.get().getUserSchema(user, c, BioTrustQuerySchema.NAME);
        TableInfo statusTable = schema.getTable(BioTrustQuerySchema.REQUEST_STATUS_TABLE_NAME);

        SimpleFilter filter = new SimpleFilter();
        filter.addCondition(FieldKey.fromParts("LockedState"), true);
        return new TableSelector(statusTable, Collections.singleton("Status"), filter, null).getArrayList(String.class);
    }
}
