...
select *,
  ...
  END as percent,
  case
    when aa.q_aa LIKE '%:%' then null
    when aa.depth > 0 THEN
      round(((aa.total_reads-aa.incompletecodons) / (aa.depth-aa.incompletecodons))*100, 2)
    ELSE 0
  END as adj_percent,
  cast(case
    when aa.q_aa = ':' then 0
    when aa.q_aa like '%:%' then (aa.total_reads-aa.incompletecodons)
    else aa.total_reads
  end  as integer) as adj_total_reads,
...
SELECT
  ...
  aa.q_aa,
  cast(aa.total_reads as integer) as total_reads,
...
--   aa.adj_depth,
  ...
  WHERE
      ...
      a.ref_aa_insert_index = aa.ref_aa_insert_index AND
      a.q_aa = ':'
  ) as incompletecodons
...
SELECT
  ...
  aa.ref_nt_id,
  aa.ref_aa_id,
  group_concat(DISTINCT aa.ref_aa) AS ref_aa,
  group_concat(DISTINCT aa.nt_snp_id.ref_nt_position) AS ref_nt_positions,
  group_concat(DISTINCT aa.nt_snp_id) AS nt_snp_ids,
  aa.ref_aa_position,
  aa.ref_aa_insert_index,
