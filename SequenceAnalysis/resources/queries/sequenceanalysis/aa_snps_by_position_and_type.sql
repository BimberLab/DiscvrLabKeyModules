...
group_concat(DISTINCT CASE
  ...
  WHEN c.ref_aa != c.q_aa THEN 'Non-Synonymous'
END, ',') as mutationType,
...
100.0 * (CAST(sum(CASE WHEN (c.q_aa = ':' OR c.q_aa = '?' OR c.q_aa = '+' OR c.ref_aa = '-') THEN c.readcount ELSE 0 END) AS DOUBLE) / max(c.adj_depth)) as indel_fraction,

FROM sequenceanalysis.aa_snps_by_codon c

GROUP BY c.analysis_id, c.ref_nt_id, c.ref_aa_id, c.ref_aa, c.ref_aa_position, c.ref_aa_insert_index, (CASE
  WHEN (c.q_aa = ':' OR c.q_aa = '?' OR c.q_aa = '+' OR c.ref_aa = '-') THEN 'Indel'
  WHEN c.ref_aa = c.q_aa THEN 'Synonymous'
  WHEN c.ref_aa != c.q_aa THEN 'Non-Synonymous'
END)
