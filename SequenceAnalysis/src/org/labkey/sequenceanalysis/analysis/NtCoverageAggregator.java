...
public class NtCoverageAggregator extends AbstractAlignmentAggregator
{
    ...
    public NtCoverageAggregator(SequencePipelineSettings settings, Logger log, AvgBaseQualityAggregator avgQualAggregator)
    {
    ...
    private void appendSnp(String refName, int position, int index, int qual, char base, Map<String, int[][]> coverageMap, Map<String, int[][]> qualMap, Map<String, int[][][]> coverageByBase, Map<String, int[][][]> qualByBase)
    {
        ...
        if (qualByBase != null)
        {
            ...
            qualTotalByBase[index][baseIndex]++;
        }
    }
    ...
    {
        _log.info("Saving Coverage Results to DB");
        ...
        try (DbScope.Transaction transaction = ExperimentService.get().ensureTransaction())
        {
            ...
            for (String refName : _totalCoverage.keySet())
            {
                ...
                int refId = Integer.parseInt(names[0]);
                ...
                for (int position = 0; position < coverageForRef.length; position++)
                {
                    ...
                    for (int index = 0; index < coverageByIndex.length; index++)
                    {
                        ...
                        for (char base : _baseMap.keySet())
                        {
                            ...
                            row.put("avgqual_" + fieldSuffix, getValueForPositionAndBase(refName, position, index, base, _totalQualByBase));
                        }
                        ...
                        if (savedHCDepth > 0)
                        {
                            ...
                            {
                                _log.error("High quality coverage total doesnt match at position " + position + ": " + totalWithoutN + " / " + savedHCDepth);
                            }
                        ...
                        if (total != savedDepth)
                        {
                            _log.error("Coverage doesnt match " + position + ": " + total + " / " + savedDepth);
                        }
            ...
            _log.info("\tPositions saved by reference (may include indels, so total could exceed reference length):");
